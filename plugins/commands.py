...
async def start(client:Client, message): 
    ...
    if len(m.command) == 2 and m.command[1].startswith('notcopy'):
        try:
            # FIX: Naya format (group id ke saath)
        ...
        settings = await get_settings(grp_id) # Sahi group ki settings load karega
        ...
        try:
            ...
            logger.warning(f"Verification log bhej nahi paaya: {e}")

        # --- FIX: Naya 'get_' link (group id ke saath) generate karein ---
        btn = [[
            InlineKeyboardButton("✅ ᴄʟɪᴄᴋ ʜᴇʀᴇ ᴛᴏ ɢᴇᴛ ꜰɪʟᴇ / ɴᴇxᴛ sᴛᴇᴘ ✅", url=f"https://telegram.me/{temp.U_NAME}?start=get_{grp_id_str}_{link_id}"),
        ]]
    ...
    if data.startswith("get_"):
        try:
            # FIX: Naya format: get_GROUPID_LINKID
        ...
        if not await db.has_premium_access(user_id):
            # FIX: Ab hum sahi group ki settings check kar rahe hain
            settings = await get_settings(grp_id)
            
            # --- 3-STEP VERIFICATION LOGIC (Bug Fixed) ---
            if settings.get("is_verify", IS_VERIFY): # <-- Yeh group ki setting check karega
                ...
                if v3_time > v1_time and v3_time > v2_time:
                    ...
                    # FIX: 60 sec grace period for one-time links
                    if duration_seconds == 0: 
                        if time_since_v3 < 60: show_link = False
                ...
                if show_link:
                    ...
                    # FIX: Naya verification link (Group ID aur Link ID dono ke saath)
    ...
    # Fallback (Agar user koi galat link daalta hai)
    buttons = [[InlineKeyboardButton('⇆ ᴀᴅᴅ ᴍᴇ ᴛᴏ ʏᴏᴜʀ ɢʀᴏᴜᴘs ⇆', url=f'http://t.me/{temp.U_NAME}?startgroup=start')]]
    reply_markup = InlineKeyboardMarkup(buttons)
    await message.reply_text(script.START_TXT.format(message.from_user.mention, get_status(), message.from_user.id),
        reply_markup=reply_markup, parse_mode=enums.ParseMode.HTML)
...
async def delete(bot, message):
    ...
    # --- FIX: Naya delete logic ---
    try:
        # File ka link_id (jo master list ka _id hai) nikaalein
        link_id, file_ref = unpack_new_file_id(media.file_id)
        
        # 1. Master list (files_data) se delete karein
        result1 = await FilesData.collection.delete_one({'_id': link_id})
        ...
        if deleted_count > 0:
            ...
            await msg.edit('<b>File database mein nahi mili.</b>\n(Ho sakta hai yeh file puraane system se index ho)')
            
    except Exception as e:
        logger.error(f"Delete error: {e}")
        await msg.edit(f"<b>Delete karte waqt error:</b> <code>{e}</code>\n(Shayad yeh file naye system se index nahi hai)")


# --- /deleteall COMMAND (FIXED) ---
@Client.on_message(filters.command('deleteall'))
async def delete_all_index(bot, message):
    if message.from_user.id not in ADMINS: return
    # FIX: Yeh ab sirf search index (Media) ko delete karega.
    files = await Media.count_documents() # Media = MediaSecondary
    if int(files) == 0:
        return await message.reply_text('Search Index (DB2) mein delete karne ke liye files nahi hain.')
    btn = [[InlineKeyboardButton(text="ʏᴇs (Delete DB2 Search Index)", callback_data="all_files_delete")], [InlineKeyboardButton(text="ᴄᴀɴᴄᴇʟ", callback_data="close_data")]]
...
# --- /settings COMMAND (Waise hi rahega) ---
...
# --- Baaki saare commands (/set_template, /send, #request, etc.) waise hi rahenge ---
...
async def search_files(bot, message):
    ...
    files, total = await get_bad_files(keyword) # Yeh search index se dhoondhega
    if int(total) == 0:
...
async def deletemultiplefiles(bot, message):
    ...
    await message.reply_text(text=f"<b>ᴛᴏᴛᴀʟ ꜰɪʟᴇs ꜰᴏᴜɴᴅ - <code>{total}</code>\n\n(Yeh search index se delete karega)</b>", reply_markup=InlineKeyboardMarkup(btn), parse_mode=enums.ParseMode.HTML)
...
async def delete_files(bot, message):
    ...
    for keyword in keywords:
        result = await Media.collection.delete_many({'file_name': keyword.strip()}) # Sirf DB2 se
