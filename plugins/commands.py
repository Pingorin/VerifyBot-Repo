...
async def start(client:Client, message): 
    ...
    if len(m.command) == 2 and m.command[1].startswith('notcopy'):
        try:
            ...
            level = int(level)
            user_id = int(userid)
            grp_id = int(grp_id_str) # <-- FIX: Ab hum Group ID ko link se le rahe hain
        except Exception:
    ...
    if message.chat.type in [enums.ChatType.GROUP, enums.ChatType.SUPERGROUP]:
        # ... (Group start logic waise hi rahega) ...
        status = get_status()
    ...
    if len(message.command) != 2:
        # ... (PM start menu logic waise hi rahega) ...
        buttons = [[
    ...
    if AUTH_CHANNEL or AUTH_CHANNEL_2 or AUTH_CHANNEL_3:
        ...
        if not all_joined:
            btn = [] # ... (Poora FSub button logic waise hi rahega) ...
    ...
    if len(message.command) == 2 and message.command[1] in ["subscribe", "error", "okay", "help", "buy_premium"]:
        # ... (Premium button logic waise hi rahega) ...
    ...
    if data.startswith("get_"):
        try:
            # Naya format: get_GROUPID_LINKID
