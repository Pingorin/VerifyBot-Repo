...
async def start(client:Client, message): 
    ...
    if len(m.command) == 2 and m.command[1].startswith('notcopy'):
        ...
        # Log channel mein message (settings se log channel ID lein)
        try:
...
async def delete(bot, message):
    ...
    try:
        ...
        # Hamara link_id files_data collection ka _id hai
...
# --- Baaki saare commands (/deleteall, /settings, etc.) waise hi rahenge ---
...
async def combined_chat_member_handler(client: Client, member: ChatMemberUpdated):
    # (Yeh poora function waise hi rahega, FSub Cleanup + Referral)
    ...
    try:
        if (
            ...
            and (not member.old_chat_member or member.old_chat_member.status in [enums.ChatMemberStatus.LEFT, enums.ChatMemberStatus.BANNED]) # FIX: Sahi Enums
        ):
