...
from database.ia_filterdb import Media, get_search_results, get_bad_files, get_available_qualities, get_available_years # get_file_details ko hata diya gaya hai kyunki ab use nahi hota
...
async def group_search(client, message):
    ...
    if settings["auto_filter"]:
        ...
        if message.text.startswith("/"):
            ...
            # ... (Aapka admin report logic yahaan) ...
            pass
        else:
            await auto_filter(client, message)   
    else:
...
async def next_page(bot, query):
    ...
    # --- YEH HAI FIX ---
    # Key se grp_id nikaalein (key format: "{chat_id}-{message_id}")
    try:
        grp_id_str = key.split('-')[0]
    except:
        grp_id_str = str(query.message.chat.id) # Fallback
    # --- FIX KHATAM ---
    ...
    # temp.FILES_ID[key] = files # Iski zaroorat nahi hai agar batch send nahi kar rahe
    ...
    if settings["link"]:
        ...
        for file_num, file in enumerate(files, start=offset+1):
            # Naya link format: get_GROUPID_LINKID
            links += f"""<b>\n\n{file_num}. <a href=https://t.me/{temp.U_NAME}?start=get_{grp_id_str}_{file.link_id}>[{get_size(file.file_size)}] {' '.join(filter(lambda x: not x.startswith('[') and not x.startswith('@') and not x.startswith('www.'), file.file_name.split()))}</a></b>"""
    else:
        btn = [[InlineKeyboardButton(
                    text=f"🔗 {get_size(file.file_size)}≽ {get_name(file.file_name)}", 
                    # Naya link format: get_GROUPID_LINKID
                    url=f'https://telegram.dog/{temp.U_NAME}?start=get_{grp_id_str}_{file.link_id}'
                ),]
                for file in files
              ]
    # --- FIX KHATAM ---
    ...
    btn.insert(0,[
        # InlineKeyboardButton("♻️ sᴇɴᴅ ᴀʟʟ", callback_data=batch_link), # Batch send abhi disabled hai
...
async def lang_search(client: Client, query: CallbackQuery):
    ...
    # batch_ids = files ... (Batch logic disabled)
...
async def advantage_spoll_choker(bot, query):
    ...
    if files:
        ...
        await auto_filter(bot, query, k) # auto_filter ab updated hai
    else:
...
async def cb_handler(client: Client, query: CallbackQuery):
    ...
    # --- YEH ZAROORI HAI: STREAM LOGIC ---
        ...
        try: 
            _, link_id = query.data.split("#", 1)
        except: 
            return await query.answer("Invalid stream link!")
        ...
        AKS = await client.send_cached_media(
            ...
            file_ref=file_data.file_ref # file_ref add karein
        )
    ...
    # ... (start, features, earn, ... baaki saare menu buttons waise hi rahenge) ...
    # ... (all_files_delete, killfilesak, reset_grp_data, setgs logic waise hi rahega) ...
    # ... (Request handlers: show_options, reject, accept, etc. waise hi rahenge) ...

    elif query.data.startswith("batchfiles"):
        # (Yeh logic abhi ke liye zaroori nahi hai kyunki 'allfiles' disabled hai, 
        # lekin ise rakhne se koi nuksaan nahi)
...
async def auto_filter(client, msg, spoll=False):
    if not spoll:
        message = msg
        search = message.text
        chat_id = message.chat.id # <-- YEH GROUP ID YA USER ID HAI
        settings = await get_settings(chat_id)
        files, offset, total_results = await get_search_results(search)
        if not files:
        ...
        chat_id = msg.message.chat.id # <-- YEH GROUP ID HAI
        search, files, offset, total_results = spoll
        available_qualities = await get_available_qualities(search)
        available_years = await get_available_years(search)

    req = message.from_user.id if message.from_user else 0
    key = f"{chat_id}-{message.id}" # <-- chat_id ka istemaal karein
    
    # temp.FILES_ID[f"{message.chat.id}-{message.id}"] = batch_ids # Batch logic disabled
    # batch_link = f"batchfiles#{message.chat.id}#{message.id}#{message.from_user.id}"
    
    temp.CHAT[message.from_user.id] = chat_id
    del_msg = f"\n\n<b>⚠️ ᴛʜɪs ᴍᴇssᴀɢᴇ ᴡɪʟʟ ʙᴇ ᴀᴜᴛᴏ ᴅᴇʟᴇᴛᴇ...</b>" if settings["auto_delete"] else ''
    links = ""
    ...
    if settings["link"]:
        ...
        for file_num, file in enumerate(files, start=1):
            # Naya link format: get_CHATID_LINKID
            links += f"""<b>\n\n{file_num}. <a href=https://t.me/{temp.U_NAME}?start=get_{chat_id}_{file.link_id}>[{get_size(file.file_size)}] {' '.join(filter(lambda x: not x.startswith('[') and not x.startswith('@') and not x.startswith('www.'), file.file_name.split()))}</a></b>"""
    else:
        btn = [[InlineKeyboardButton(
                    text=f"🔗 {get_size(file.file_size)}≽ {get_name(file.file_name)}", 
                    # Naya link format: get_CHATID_LINKID
                    url=f'https://telegram.dog/{temp.U_NAME}?start=get_{chat_id}_{file.link_id}'
                ),]
               for file in files
              ]
    # --- FIX KHATAM ---
    ...
    btn.append([InlineKeyboardButton("💰 ʀᴇꜰᴇʀ & ᴇᴀʀɴ 💰", url=f"https://t.me/{temp.U_NAME}?start=get_referral_{chat_id}")])
    btn.append([InlineKeyboardButton("🤔 ʜᴏᴡ ᴛᴏ ᴅᴏᴡɴʟᴏᴀᴅ 🤔", url=settings['tutorial'])])
