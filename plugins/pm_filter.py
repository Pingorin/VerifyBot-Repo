...
async def lang_search(client: Client, query: CallbackQuery):
    ...
    if settings["link"]:
        ...
        # --- THIS IS THE FIX ---
        # Changed callback_data to url to trigger the /start command, matching other functions.
        btn = [[
...
async def cb_handler(client: Client, query: CallbackQuery):
    if query.data == "close_data":
        ...
        reply_markup = InlineKeyboardMarkup(btn)
        # --- FIX: Use reply_photo, not edit_text, if QR_CODE is a photo ---
        try:
            ...
            await query.message.delete() # Delete the original "/start" message
        except Exception as e:
        ...
        files = await Media.count_documents() # This will count from MediaSecondary
        await query.answer('Deleting from Secondary DB...')
        await Media.collection.drop() # This will drop MediaSecondary
        await query.message.edit_text(f"Successfully deleted {files} files from Secondary DB")
        ...
        async with lock:
            try:
                for file in files:
                    ...
                    # This will delete from all 4 DBs since get_bad_files searches all
                    # A bit inefficient, but will work.
                    await MediaPrimary.collection.delete_one({'_id': file_ids})
                    await MediaSecondary.collection.delete_one({'_id': file_ids})
                    await MediaThird.collection.delete_one({'_id': file_ids})
                    await MediaFourth.collection.delete_one({'_id': file_ids})
